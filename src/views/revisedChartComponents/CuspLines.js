...
export default function CuspLines(props) {
  ...
  const getStrokeColor = (cuspId) => {
    if (props.zodiacal || angularCusps.indexOf(cuspId) >= 0)
      return "black";
  ...
  const getStrokeWidth = (cuspId) => {
    if (angularCusps.indexOf(cuspId) >= 0)
      return props.zodiacal ? 2 : 1;
    if (succedentCusps.indexOf(cuspId) >= 0)
      return props.zodiacal ? 1 : 0.15;
  ...
  const cuspLine = (pos, cuspId) => {
    const [x1, y1] = derivePoint(props.origin, pos, houseRingInnerRadius, props.cuspOffset);
    const [x2, y2] = derivePoint(props.origin, pos, signRingInnerRadius, props.cuspOffset);
    return (
